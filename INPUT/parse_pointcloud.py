...
def get_2d_voxels_old(points, voxel_size = 1, n_points_in_voxel = 4096): # n_points in original pointnet is 4096
    ...
    for x_bin in range(x_bins):
         for y_bin in range(y_bins):
            ...
            if points_in_voxel.shape[0] > 120 : # check if there are any points in the voxel
...
def get_3d_voxels(points, voxel_size, n_points_in_voxel = 4096, normalized = True, show_boundary = False): # n_points in original pointnet is 4096
    '''
    ...
    #     n_features += 3
    #     points_norm_scene_location = points[:,:3] / np.max (points[:,:3], axis = 0) # normalized relative position of points to scene
    #     points = np.concatenate([
    #         points[:,:-1], # points with computed features
    #         points_norm_scene_location, 
    #         points[:,-1][:,np.newaxis] # labels
    #     ],axis=1)
        
    voxels = (np.ceil(points[:,:3]/voxel_size)).astype(int) # find the voxel bin id of each point
             ...
             for z_bin in range(z_bins):
                ...
                if points_in_voxel.shape[0] > 250 : # check if there are any points in the voxel
                    ...
                    if normalized:
                        ...
                        voxel_origin = np.min(points_in_voxel[:,:3], axis=0) + (np.array(*voxel_size)/2) * np.array([1.,1.,0])
